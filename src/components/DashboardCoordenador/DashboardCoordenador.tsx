...
const TelaSolicitacoes: React.FC<{ coordenador: CoordenadorDTO | null }> = ({ coordenador }) => {
    ...
    const statusBadge = (status: string) => {
        const map: Record<string, string> = {
            ...
            RECUSADO: 'badge-recusado', RESOLVIDA: 'badge-resolvida',
        };
        ...
        <main className="main-content">
            ...
            <section className="summary-cards">
                <div className="card">
                    ...
                    <div className="card-info"><span className="label">Total</span><span className="value">{solicitacoes.length}</span><span className="desc">Solicitações recebidas</span></div>
                </div>
                <div className="card">
                    <div className="card-icon yellow"><FileText size={24} /></div>
                    <div className="card-info"><span className="label">Pendentes</span><span className="value">{solicitacoes.filter(s => s.status === 'PENDENTE').length}</span><span className="desc">Aguardando análise</span></div>
                </div>
                <div className="card">
                    <div className="card-icon green"><CheckCircle size={24} /></div>
                    <div className="card-info"><span className="label">Aprovadas</span><span className="value">{solicitacoes.filter(s => s.status === 'APROVADO').length}</span><span className="desc">Substituição confirmada</span></div>
                </div>
                <div className="card">
                    <div className="card-icon red"><XCircle size={24} /></div>
                    <div className="card-info"><span className="label">Recusadas</span><span className="value">{solicitacoes.filter(s => s.status === 'RECUSADO').length}</span><span className="desc">Não autorizadas</span></div>
                </div>
            </section>
            <section className="panel">
                <div className="panel-header"><h3>Lista de Solicitações</h3></div>
                ...
                {!loading && !erro && (
                    <table className="schedule-table">
                        <thead>
                        <tr>
                            <th>ID</th><th>Data Ausência</th><th>Motivo</th>
                            <th>Prof. ID</th><th>Aula ID</th><th>Status</th><th>Ações</th>
                        </tr>
                        </thead>
                        <tbody>
                        {solicitacoes.length > 0 ? solicitacoes.map((s) => (
                            <tr key={s.id}>
                                ...
                                <td>{s.motivo}</td>
                                <td>#{s.professor.id}</td>
                                <td>#{s.aula.id}</td>
                                <td><span className={statusBadge(s.status)}>{s.status}</span></td>
                                <td className="acoes-cell">
                                    {s.status === 'PENDENTE' && (
                                        <>
                                            <button className="btn-aprovar" onClick={() => handleAcao(s.id, 'APROVADO')}><CheckCircle size={16} /> Aprovar</button>
                                            <button className="btn-recusar" onClick={() => handleAcao(s.id, 'RECUSADO')}><XCircle size={16} /> Recusar</button>
                                        </>
                                    )}
                                    {s.status === 'APROVADO' && (
                                        <button className="btn-designar" onClick={() => abrirModalDesignar(s)}><UserPlus size={16} /> Designar Substituto</button>
                                    )}
                            ...
                            <tr><td colSpan={7} style={{ textAlign: 'center', padding: '24px', color: '#64748b' }}>Nenhuma solicitação encontrada.</td></tr>
                        )}
                        </tbody>
                    </table>
                )}
            ...
            {modalAberto && solicitacaoSelecionada && (
                <div className="modal-overlay" onClick={() => setModalAberto(false)}>
                    <div className="modal-box" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="modal-sub">Solicitação #{solicitacaoSelecionada.id} — Ausência em {formatarData(solicitacaoSelecionada.dataAusencia)}</p>
                        <div className="modal-info-row">
                            <span className="modal-info-label">Motivo</span>
                            <span className="modal-info-value">{solicitacaoSelecionada.motivo}</span>
                        </div>
                        <div className="modal-info-row">
                            <span className="modal-info-label">Aula ID</span>
                            <span className="modal-info-value">#{solicitacaoSelecionada.aula.id}</span>
                        </div>
                        <form onSubmit={handleDesignarSubstituto}>
                            <div className="form-group-modal">
                                <label>Professor Substituto</label>
                                <select value={professorSubstitutoId} onChange={e => setProfessorSubstitutoId(e.target.value)} required>
...
const TelaSubstituicoes: React.FC = () => {
    ...
    return (
        <main className="main-content">
            ...
            <section className="panel">
                <div className="panel-header"><h3>Lista de Substituições ({substituicoes.length})</h3></div>
                {loading && <p className="loading-text">Carregando...</p>}
                {erro && <div className="alert-error"><strong>🚨</strong> {erro}</div>}
                {!loading && !erro && (
                    <table className="schedule-table">
...
const TelaUsuarios: React.FC = () => {
    ...
    return (
        <main className="main-content">
            <header className="top-header">
                <div className="header-title"><h1>Todos os Usuários</h1><p>Visualize todos os professores e coordenadores cadastrados no sistema.</p></div>
                <div className="header-actions"><div className="avatar-top"><User size={20} color="#fff" /></div></div>
            </header>
            {loading && <p className="loading-text">Carregando...</p>}
            {erro && <div className="alert-error"><strong>🚨</strong> {erro}</div>}
            {!loading && !erro && (
                <>
                    <section className="panel" style={{ marginBottom: '24px' }}>
                        ...
                        <table className="schedule-table">
                            <thead><tr><th>ID</th><th>Nome</th><th>E-mail</th><th>Matrícula</th><th>Área</th><th>Status</th></tr></thead>
                            <tbody>
                            {professores.map((p, i) => (
                                <tr key={i}>
                    ...
                    <section className="panel">
                        <div className="panel-header"><h3>Coordenadores ({coordenadores.length})</h3></div>
                        <table className="schedule-table">
                            <thead><tr><th>ID</th><th>Nome</th><th>E-mail</th><th>Matrícula</th><th>Status</th></tr></thead>
                            <tbody>
                            {coordenadores.map((c, i) => (
                                <tr key={i}>
                                    <td>#{c.id}</td><td>{c.nome}</td><td>{c.email}</td><td>{c.matricula}</td>
